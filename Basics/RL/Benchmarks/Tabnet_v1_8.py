...
# -*- coding: utf-8 -*-
"""
tabnet_general_csv_benchmark_v8_ultrafast.py

Versão 8 — Ultrafast, com correções de espaço de busca e perda híbrida.
Uso:
 python tabnet_general_csv_benchmark_v8_ultrafast.py --train creditcard_train.csv --val creditcard_val.csv --test creditcard_test.csv --max_eval 10
"""
import os
import time
import json
...
def train_tabnet(Xtr, ytr, Xval=None, yval=None, params=None, seed=42):
    ...
    tabnet_kwargs = dict(
        ...
        n_a=int(params.get('n_a', 16)),
        n_steps=int(params.get('n_steps', 4)),
        gamma=float(params.get('gamma', 1.3)),
        lambda_sparse=float(params.get('lambda_sparse', 1e-5)),
        optimizer_fn=torch.optim.Adam,
        optimizer_params=dict(lr=float(params.get('lr', 1e-3)), weight_decay=float(params.get('weight_decay', 1e-6))),
        mask_type='sparsemax',
        seed=seed,
        verbose=1
    )
    ...
    if spw != 1.0:
        # multiply weight of positive class (but kept limited by search space)
...
def build_space():
    space = {
        # architecture (indices -> mapped)
        "n_d": hp.choice("n_d", [0, 1]),           # -> [16,24]
        "n_a": hp.choice("n_a", [0, 1]),           # -> [16,24]
        "n_steps": hp.choice("n_steps", [0]),      # -> [4]
        # continuous
        ...
        # scale pos weight
        "scale_pos_weight_choice": hp.choice("scale_pos_weight_choice", [0,1,2])
    }
    return space
...
# LIMITED SP_OPTIONS: avoid extreme values that break precision
SP_OPTIONS = [
    {'type': 'fixed', 'value': 1.0},
    {'type': 'fixed', 'value': 20.0},   # previously 500.0 -> reduced to 20
    {'type': 'auto'}                    # computed from data, bounded by common sense
]

# -------------------------
# Objective (hybrid metric: AUC + AP)
# -------------------------
def objective(space_sample, Xtr, ytr, Xval, yval, trial_logger=None, seed=42):
    ...
    if sp_sel['type'] == 'auto':
        neg = (ytr == 0).sum()
        pos = (ytr == 1).sum()
        computed_sp = float(neg / max(1, pos))
        # safety cap: don't allow absurdly large values in auto (cap at 20)
        params['scale_pos_weight'] = min(computed_sp, 20.0)
    else:
    ...
    try:
        ...
        # if training crashes for a trial, log and return high loss
        record_err = {
        ...
        if trial_logger is not None:
            trial_logger.append_record(record_err)
        return {"loss": 1.0, "status": STATUS_OK, "attachments": {"record": record_err}}

    t1 = time.time()

    # Evaluate on validation
    yval_prob = clf.predict_proba(Xval)[:, 1]
    val_auc = roc_auc_score(yval, yval_prob) if len(np.unique(yval)) > 1 else 0.0
    val_ap = average_precision_score(yval, yval_prob) if len(np.unique(yval)) > 1 else 0.0

    # hybrid metric (weighted)
    search_metric = 0.7 * val_auc + 0.3 * val_ap
    ...
    return {"loss": -float(search_metric), "status": STATUS_OK, "attachments": {"record": record}}
...
def reconstruct_best_params(best_dict, trials_obj, Xtr_y=None):
    ...
    # fallback: map indices to CHOICE_MAP
    bp = {}
    ...
    if 'scale_pos_weight_choice' in best_dict:
        ...
        if sp_sel['type'] == 'auto':
            if Xtr_y is not None:
                ...
                neg = (ytr == 0).sum()
                pos = (ytr == 1).sum()
                bp['scale_pos_weight'] = min(float(neg / max(1, pos)), 20.0)
            else:
...
def main(args):
    ...
    meta = {
        ...
        "best_estimated_search_metric": float(-min([r.get('loss', 0) for r in trials.results])) if trials.results else None
    }
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="TabNet hyperopt benchmark V8 - Ultrafast")
    parser.add_argument("--train", required=True)
    parser.add_argument("--val", required=True)
    parser.add_argument("--test", required=True)
    parser.add_argument("--output_dir", default="outputs_tabnet_v8")
